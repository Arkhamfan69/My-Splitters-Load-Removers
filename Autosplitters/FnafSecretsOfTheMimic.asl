...
init
{
    ...
    vars.Helper["SaveID"] = vars.Helper.Make<ulong>(gEngine, 0xD28, 0x38, 0x0, 0x30, 0x2A0, 0x71C);
...
update
{
    ...
    if (old.TransitionState != current.TransitionState)
    {
        vars.Log("TransitionState: " + old.TransitionState + " -> " + current.TransitionState);
    }
